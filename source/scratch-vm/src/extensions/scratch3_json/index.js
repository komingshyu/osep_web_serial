...
class gasoJSON {
    ...
    getInfo() {
        ...
        return {
            ...
            blocks: [
                ...
                "---",
                {
                    opcode: "openexceltojsonurl",
                    blockType: BlockType.COMMAND,                    
                    text: msg.openexceltojsonurl[theLocale],
                },
                {
                    opcode: "texttojson",
                    blockType: BlockType.COMMAND,
                    arguments: {
                        TEXT: {
                            type: ArgumentType.STRING,
                            defaultValue: '[{"id":1,"name":"john"},{"id":2,"name":"marry"}]',
                        },                        
                    },
                    text: msg.texttojson[theLocale],
                },
                {
                    opcode: "editjson",
                    blockType: BlockType.COMMAND,
                    ...
                    text: msg.editjson[theLocale],
                },
                {
                    opcode: "jsonclear",
                    blockType: BlockType.COMMAND,
                    text: msg.jsonclear[theLocale],
                },
                /*{
                    opcode: "jsonadd",
                    blockType: BlockType.COMMAND,
                ...
                },                
                {
                    ...
                    text: msg.jsondeletenum[theLocale],
                },
                {
    ...
    async texttojson(args){
        //const result_text= JSON.stringify(JSON.parse(args.TEXT));
        this.data.data = JSON.parse(JSON.stringify(args.TEXT));
    ...
    openexceltojsonurl(){
        window.open('https://estea8968.github.io/cpexcel2json/app/','_blank');
    }
    ...
    jsondeletenum(args){
        ...
        let data_array =JSON.parse(this.data.data);
        //移除最後一筆資料
        data_array.splice(num-1,1);
        this.data.data = JSON.stringify(data_array);
        console.log(this.data.data);
    }
    editjson(args){
        const row=parseInt(args.ROW,10);
        const col=args.COL;
        const in_val=args.VALUE;
        console.log(row,col,in_val);
        let data_array =JSON.parse(this.data.data);
        for(i=0;i<data_array.length;i++){
            if(i===(row-1)){
                const obj_ary=Object.keys(data_array[i]);
                data_array[i][col]=in_val;
                console.log('edit val',data_array[i]);
            }            
        }
        this.data.data = JSON.stringify(data_array);
        console.log(this.data.data);
    ...
    /*jsonadd(args){
        const in_array=JSON.parse(args.TEXT);
        console.log('in_array=',in_array)
        let json_array=JSON.parse(this.data.data);
        console.log('json_array=',json_array);
        for(i=1;i<json_array.length+1;i++){
            in_array[i]=json_array[i-1];
        }
        //json_array[json_array.length]=in_array;
        console.log('in_array=',in_array);
        //this.data.data = JSON.stringify(JSON.parse(JSON.stringify(json_array)));
        this.data.data = JSON.stringify(in_array);
        console.log('this.data.data =',this.data.data);        
        return this.data.data; 
    }*/
